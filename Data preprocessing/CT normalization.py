...
def axial_slice_three(resampled_root, np_root, roi_center_per_patient_dict, cut_size = 64):
    patient_no = os.listdir(resampled_root)
    for no in patient_no :
        patient_root = os.path.join(resampled_root, no)
        patient_data = os.listdir(patient_root)
...
def main():
    ...
    parser.add_argument(
        "--result",
        default="/USER/USER_WORKSPACE/jechoi/code/result",
        type=str,
        help="model is saving in this directory"
    )
    ...
    thick_count = image_slice_check(ct_data)
    ...
    axial_slice_three(resampled_root, np_root, roi_center_per_patient_dict, cut_size = 64)
